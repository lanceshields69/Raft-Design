(()=>{var j=Object.defineProperty;var Y=(t,e,i)=>e in t?j(t,e,{enumerable:!0,configurable:!0,writable:!0,value:i}):t[e]=i;var o=(t,e,i)=>Y(t,typeof e!="symbol"?e+"":e,i);var A=`#version 300 es
...
`,F={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},P={random:1,"2x2":2,"4x4":3,"8x8":4};function z(t){if(Array.isArray(t))return t.length===4?t:t.length===3?[...t,1]:b;if(typeof t!="string")return b;let e,i,r,a=1;if(t.startsWith("#"))[e,i,r,a]=Z(t);else if(t.startsWith("rgb")){let s=J(t);if(s===null)return b;[e,i,r,a]=s}else if(t.startsWith("hsl")){let s=Q(t);if(s===null)return b;[e,i,r,a]=ee(s)}else return console.error("Unsupported color format",t),b;return[w(e,0,1),w(i,0,1),w(r,0,1),w(a,0,1)]}function Z(t){if(t=t.replace(/^#/,""),(t.length===3||t.length===4)&&(t=t.split("").map(s=>s+s).join("")),t.length===6&&(t=t+"ff"),!/^[0-9a-f]{8}$/i.test(t))return console.warn("Invalid hex color"),b;let e=parseInt(t.slice(0,2),16)/255,i=parseInt(t.slice(2,4),16)/255,r=parseInt(t.slice(4,6),16)/255,a=parseInt(t.slice(6,8),16)/255;return[e,i,r,a]}function J(t){var i,r,a;let e=t.match(/^rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([0-9.]+))?\s*\)$/i);return e?[parseInt((i=e[1])!=null?i:"0")/255,parseInt((r=e[2])!=null?r:"0")/255,parseInt((a=e[3])!=null?a:"0")/255,e[4]===void 0?1:parseFloat(e[4])]:null}function Q(t){var i,r,a;let e=t.match(/^hsla?\s*\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([0-9.]+))?\s*\)$/i);return e?[parseInt((i=e[1])!=null?i:"0"),parseInt((r=e[2])!=null?r:"0"),parseInt((a=e[3])!=null?a:"0"),e[4]===void 0?1:parseFloat(e[4])]:null}function ee(t){let[e,i,r,a]=t,s=e/360,l=i/100,n=r/100,h,u,p;if(i===0)h=u=p=n;else{let c=(d,S,m)=>(m<0&&(m+=1),m>1&&(m-=1),m<.16666666666666666?d+(S-d)*6*m:m<.5?S:m<.6666666666666666?d+(S-d)*(.6666666666666666-m)*6:d),f=n<.5?n*(1+l):n+l-n*l,v=2*n-f;h=c(v,f,s+1/3),u=c(v,f,s),p=c(v,f,s-1/3)}return[h,u,p,a]}var w=(t,e,i)=>Math.min(Math.max(t,e),i),b=[.5,.5,.5,1];var H=window.matchMedia("(prefers-reduced-motion: reduce)").matches,g=V,T=document.documentElement,G=t=>{let i=T.getAttribute("data-theme")==="light"&&t.dataset.lightFront?t.dataset.lightFront:"--bg-brand";return getComputedStyle(T).getPropertyValue(i).trim()||"#3DE83D"};document.querySelectorAll("[data-dither-banner], [data-dither]").forEach(t=>{let e=t.dataset,i=(a,s)=>a===void 0?s:parseFloat(a),r=e.messages?JSON.parse(e.messages):null;try{let a=new U(t,E,{u_colorBack:z("#00000000"),u_colorFront:z(G(t)),u_shape:F[e.shape||"wave"],u_type:P[e.type||"4x4"],u_pxSize:i(e.size,3),u_fit:y[g.fit],u_scale:i(e.scale,1.25),u_rotation:g.rotation,u_offsetX:g.offsetX,u_offsetY:g.offsetY,u_originX:g.originX,u_originY:g.originY,u_worldWidth:g.worldWidth,u_worldHeight:g.worldHeight},r?{alpha:!0,premultipliedAlpha:!0,preserveDrawingBuffer:!0}:void 0,H?0:i(e.speed,.94),i(e.frame,1097832933e-3));t.dataset.lightFront&&new MutationObserver(()=>a.setUniforms({u_colorFront:z(G(t))})).observe(T,{attributes:!0,attributeFilter:["data-theme"]}),r&&!H&&te(t,a.canvasElement,r)}catch{t.hidden=!0}});function te(t,e,i){let r=t.parentElement.querySelector(".raft-message-text");if(!r||i.length<2)return;let a=48,s=12,l=document.createElement("canvas");l.width=a,l.height=s;let n=l.getContext("2d",{willReadFrequently:!0}),h=parseFloat(t.dataset.darkBelow||"0.06"),u=parseFloat(t.dataset.litAbove||"0.25"),p=3e3,c=8e3,f=14e3,v=0,d=!1,S=performance.now(),m=!0;new IntersectionObserver(x=>{m=x[0].isIntersecting}).observe(t);let N=()=>{let x=e.width,_=e.height;if(!x||!_)return 1;n.clearRect(0,0,a,s),n.drawImage(e,x*.15,_*.35,x*.7,_*.3,0,0,a,s);let R=n.getImageData(0,0,a,s).data,M=0;for(let B=3;B<R.length;B+=4)R[B]>128&&M++;return M/(a*s)},X=()=>{v=(v+1)%i.length,r.textContent=i[v],S=performance.now(),d=!1};setInterval(()=>{if(!m||document.hidden)return;let x=N(),_=performance.now()-S;!d&&x>u&&(d=!0);let R=Math.max(0,Math.min(1,(_-c)/(f-c)));(d&&x<h&&_>p||R>0&&x<h+R*(.5-h)||_>f)&&X()},100)}})();
