...
`,V={simplex:1,warp:2,dots:3,wave:4,ripple:5,swirl:6,sphere:7},w={random:1,"2x2":2,"4x4":3,"8x8":4};function S(t){if(Array.isArray(t))return t.length===4?t:t.length===3?[...t,1]:x;if(typeof t!="string")return x;let e,i,r,a=1;if(t.startsWith("#"))[e,i,r,a]=H(t);else if(t.startsWith("rgb")){let s=G(t);if(s===null)return x;[e,i,r,a]=s}else if(t.startsWith("hsl")){let s=X(t);if(s===null)return x;[e,i,r,a]=N(s)}else return console.error("Unsupported color format",t),x;return[_(e,0,1),_(i,0,1),_(r,0,1),_(a,0,1)]}function H(t){if(t=t.replace(/^#/,""),(t.length===3||t.length===4)&&(t=t.split("").map(s=>s+s).join("")),t.length===6&&(t=t+"ff"),!/^[0-9a-f]{8}$/i.test(t))return console.warn("Invalid hex color"),x;let e=parseInt(t.slice(0,2),16)/255,i=parseInt(t.slice(2,4),16)/255,r=parseInt(t.slice(4,6),16)/255,a=parseInt(t.slice(6,8),16)/255;return[e,i,r,a]}function G(t){var i,r,a;let e=t.match(/^rgba?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*([0-9.]+))?\s*\)$/i);return e?[parseInt((i=e[1])!=null?i:"0")/255,parseInt((r=e[2])!=null?r:"0")/255,parseInt((a=e[3])!=null?a:"0")/255,e[4]===void 0?1:parseFloat(e[4])]:null}function X(t){var i,r,a;let e=t.match(/^hsla?\s*\(\s*(\d+)\s*,\s*(\d+)%\s*,\s*(\d+)%\s*(?:,\s*([0-9.]+))?\s*\)$/i);return e?[parseInt((i=e[1])!=null?i:"0"),parseInt((r=e[2])!=null?r:"0"),parseInt((a=e[3])!=null?a:"0"),e[4]===void 0?1:parseFloat(e[4])]:null}function N(t){let[e,i,r,a]=t,s=e/360,l=i/100,n=r/100,h,c,p;if(i===0)h=c=p=n;else{let u=(g,R,m)=>(m<0&&(m+=1),m>1&&(m-=1),m<.16666666666666666?g+(R-g)*6*m:m<.5?R:m<.6666666666666666?g+(R-g)*(.6666666666666666-m)*6:g),f=n<.5?n*(1+l):n+l-n*l,b=2*n-f;h=u(b,f,s+1/3),c=u(b,f,s),p=u(b,f,s-1/3)}return[h,c,p,a]}var _=(t,e,i)=>Math.min(Math.max(t,e),i),x=[.5,.5,.5,1];var j=window.matchMedia("(prefers-reduced-motion: reduce)").matches,d=U,Y=getComputedStyle(document.documentElement).getPropertyValue("--bg-brand").trim()||"#3DE83D";document.querySelectorAll("[data-dither-banner], [data-dither]").forEach(t=>{let e=t.dataset,i=(r,a)=>r===void 0?a:parseFloat(r);try{new v(t,B,{u_colorBack:S("#00000000"),u_colorFront:S(Y),u_shape:V[e.shape||"wave"],u_type:w[e.type||"4x4"],u_pxSize:i(e.size,3),u_fit:z[d.fit],u_scale:i(e.scale,1.25),u_rotation:d.rotation,u_offsetX:d.offsetX,u_offsetY:d.offsetY,u_originX:d.originX,u_originY:d.originY,u_worldWidth:d.worldWidth,u_worldHeight:d.worldHeight},void 0,j?0:i(e.speed,.94),i(e.frame,1097832933e-3))}catch{t.hidden=!0}});})();
